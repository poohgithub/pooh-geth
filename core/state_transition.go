...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	// stop burning base fees if Poohnet is active
	if rules.IsLondon && !st.evm.ChainConfig().IsPoohnet(st.evm.Context.BlockNumber) {
	...
	if st.evm.Config.NoBaseFee && msg.GasFeeCap.Sign() == 0 && msg.GasTipCap.Sign() == 0 {
		...
		if st.evm.ChainConfig().IsPoohnet(st.evm.Context.BlockNumber) {
			commonsCut := new(big.Int).Div(new(big.Int).Mul(big.NewInt(30), txFee), big.NewInt(100)) // 30% of txFee
			st.state.AddBalance(st.evm.Context.Coinbase, txFee.Sub(txFee, commonsCut))
			st.state.AddBalance(st.evm.ChainConfig().Poohnet.CommonsBudget, commonsCut)
		} else {
