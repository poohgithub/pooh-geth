...
func (c *Clique) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, withdrawals []*types.Withdrawal) {
	...
	log.Info("Clique Finalize", "header", header.Number)
	if chain.Config().IsPoohnet(header.Number) {
		if header.Number.Cmp(&chain.Config().Poohnet.LastCommonsBudgetRewardBlock) < 0 {
			state.AddBalance(chain.Config().Poohnet.CommonsBudget, &chain.Config().Poohnet.CommonsBudgetReward)
			log.Info("Clique Finalize", "header", header.Number, "add commons budget", chain.Config().Poohnet.CommonsBudgetReward)
		}
	}
